...
import (
	...
	"gateway_go/chat-room/common/util"
...
func saveMessage(message *protocol.Message) {
	...
	if message.ContentType == 2 {
		...
		err := ioutil.WriteFile(global.App.Config.Storage.Disks.LocalStorage.RootImageDir, dataBuffer, 0666)
		if err != nil {
		...
		err := ioutil.WriteFile(global.App.Config.Storage.Disks.LocalStorage.RootImageDir, message.File, 0666)
		if err != nil {
			// log.Logger.Error("write file error", log.String("write file error", err.Error()))
			return
		}
